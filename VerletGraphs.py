...
def run():
    ...
    # Loading the Data
    DataIn2500= np.load(data_path("V_1Year_2500.npy"),allow_pickle=True)
    DataIn5000 = np.load(data_path("V_1Year_5000.npy"),allow_pickle=True)
    DataIn10000 = np.load(data_path("V_1Year_10000.npy"),allow_pickle=True)
    ...
    ax[0, 0].set_title("2D Plot 1 Year Verlet (2500 s dT) ")
    ...
    for itemRow in DataIn2500:
        Ang_V_2500_list.append(itemRow[3])

    for itemRow in DataIn5000:
        Ang_V_5000_list.append(itemRow[3])

    for itemRow in DataIn10000:
        Ang_V_10000_list.append(itemRow[3])
        
    # Ensuring the arrays are numpy arrays for ease of handling.
    Ang_V_2500_list = np.array(Ang_V_2500_list)
    Ang_V_5000_list = np.array(Ang_V_5000_list)
    Ang_V_10000_list = np.array(Ang_V_10000_list)

    # Formatting the title and the graphs axis, the [0,1] indicates where on the 2x2 this plot will sit.
    ax[0,1].set(title="Total Angular Momentum Over 1 Year Verlet", xlabel="Time (s)", ylabel="Total Angular Momenrum (kg*km^2/s^2)")

    # Plots all the data on separate lines, ensuring labels and colours are correct.
    ax[0,1].plot(times,Ang_V_2500_list,label = "2500(s) dT",color = "black")
    ax[0,1].plot(times,Ang_V_5000_list,label = "5000(s) dT",color = "blue")
    ax[0,1].plot(times,Ang_V_10000_list,label = "10000(s) dT",color = "red")
    ...
    for itemRow in DataIn2500:
        Lin_V_2500_list.append(itemRow[2])

    for itemRow in DataIn5000:
        Lin_V_5000_list.append(itemRow[2])

    for itemRow in DataIn10000:
        Lin_V_10000_list.append(itemRow[2])

    # Ensuring the arrays are numpy arrays for ease of handling.
    Lin_V_2500_list = np.array(Lin_V_2500_list)
    Lin_V_5000_list= np.array(Lin_V_5000_list)
    Lin_V_10000_list= np.array(Lin_V_10000_list)

    # Setting the titles and axis names to the graph ensuring it is clear what is being displayed, the [1,0] defines its position on the 2x2 grid. 
    ax[1,0].set(title="Total Linear Momentum Over 1 Year Verlet ", xlabel="Time (s)", ylabel="Total Linear Momentum (kg*km/s)")


    # Plots all the data on separate lines, ensuring labels and colours are correct.
    ax[1,0].plot(times,Lin_V_2500_list,label = "2500(s) dT ",color = "black")
    ax[1,0].plot(times,Lin_V_5000_list,label = "5000(s) dT",color = "blue")
    ax[1,0].plot(times,Lin_V_10000_list,label = "10000(s) dT",color = "red")
    ...
    for itemRow in DataIn2500:
        En_V_2500_list.append(itemRow[1])

    for itemRow in DataIn5000:
        En_V_5000_list.append(itemRow[1])

    for itemRow in DataIn10000:
        En_V_10000_list.append(itemRow[1])

    # Ensuring the arrays are numpy arrays for ease of handling.
    En_V_2500_list = np.array(En_V_2500_list)
    En_V_5000_list = np.array(En_V_5000_list)
    En_V_10000_list = np.array(En_V_10000_list)

    # Formatting the title and the graphs axis, the [1,1] indicates where on the 2x2 this plot will sit.
    ax[1,1].set(title="Total Energy Over 1 Year Verlet ", xlabel="Time (s)", ylabel="Total Energy (10^6 J)")

    # Plots all the data on separate lines, ensuring labels and colours are correct.
    ax[1,1].plot(times,En_V_2500_list,label = "2500(s) dT ",color = "black")
    ax[1,1].plot(times,En_V_5000_list,label = "5000(s) dT",color = "blue")
    ax[1,1].plot(times,En_V_10000_list,label = "10000(s) dT",color = "red")
    ...
    # Print statements to try and make the user experience better.
    ...
    # Ensuring the arrays are numpy arrays for ease of handling.
    max_yEn10 = np.amax(En_V_10000_list)
    min_yEn10 = np.amin(En_V_10000_list)
    max_yEn5 = np.amax(En_V_5000_list)
    min_yEn5 = np.amin(En_V_5000_list)
    max_yEn25 = np.amax(En_V_2500_list)
    min_yEn25 = np.amin(En_V_2500_list)
    ...
    max_yLM10 = np.amax(Lin_V_10000_list)
    min_yLM10 = np.amin(Lin_V_10000_list)
    max_yLM5 = np.amax(Lin_V_5000_list)
    min_yLM5 = np.amin(Lin_V_5000_list)
    max_yLM25 = np.amax(Lin_V_2500_list)
    min_yLM25 = np.amin(Lin_V_2500_list)
    ...
    max_yAM10 = np.amax(Ang_V_10000_list)
    min_yAM10 = np.amin(Ang_V_10000_list)
    max_yAM5 = np.amax(Ang_V_5000_list)
    min_yAM5 = np.amin(Ang_V_5000_list)
    max_yAM25 = np.amax(Ang_V_2500_list)
    min_yAM25 = np.amin(Ang_V_2500_list)
