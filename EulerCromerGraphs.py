...
def run():
    # Loading the Data
    DataIn2500= np.load(data_path("EC_1Year_2500.npy"),allow_pickle=True)
    DataIn5000 = np.load(data_path("EC_1Year_5000.npy"),allow_pickle=True)
    DataIn10000 = np.load(data_path("EC_1Year_10000.npy"),allow_pickle=True)
    ...
    # Create a 2x2 grid of subplots for the graphs2x2 grid of subplots
    ...
    for col in range(4,(DataIn2500.shape[1]-5)): # CHANGE ONCE ADDED OTHER VALUES
        
        # Resets the xy positions for each object.
        x_positions = []
        y_positions = []

        for row in range(0,DataIn2500.shape[0]):
    ...
    ax[0, 0].set_title("2D Plot 1 Year Euler Cromer")
    ...
    for itemRow in DataIn2500:
        Ang_EC_2500_list.append(itemRow[3])

    for itemRow in DataIn5000:
        Ang_EC_5000_list.append(itemRow[3])

    for itemRow in DataIn10000:
        Ang_EC_10000_list.append(itemRow[3])
        

    # Ensuring the arrays are numpy arrays for ease of handling.
    Ang_EC_2500_list = np.array(Ang_EC_2500_list)
    Ang_EC_5000_list = np.array(Ang_EC_5000_list)
    Ang_EC_10000_list = np.array(Ang_EC_10000_list)

    # Formatting the title and the graphs axis, the [0,1] indicates where on the 2x2 this plot will sit.
    ax[0,1].set(title="Total Angular Momentum Over 1 Year Euler Cromer", xlabel="Time (s)", ylabel="Total Angular Momentum (kg^2/s)")

    # Plots all the data on separate lines, ensuring labels and colours are correct.
    ax[0,1].plot(times,Ang_EC_2500_list,label = "2500(s) dT",color = "black")
    ax[0,1].plot(times,Ang_EC_5000_list,label = "5000(s) dT",color = "blue")
    ax[0,1].plot(times,Ang_EC_10000_list,label = "10000(s) dT",color = "red")
    ...
    for itemRow in DataIn2500:
        Lin_EC_2500_list.append(itemRow[2])

    for itemRow in DataIn5000:
        Lin_EC_5000_list.append(itemRow[2])

    for itemRow in DataIn10000:
        Lin_EC_10000_list.append(itemRow[2])

    # Ensuring the arrays are numpy arrays for ease of handling.
    Lin_EC_2500_list = np.array(Lin_EC_2500_list)
    Lin_EC_5000_list= np.array(Lin_EC_5000_list)
    Lin_EC_10000_list= np.array(Lin_EC_10000_list)

    # Setting the titles and axis names to the graph ensuring it is clear what is being displayed.
    ax[1,0].set(title="Total Linear Momentum Over 1 Year Euler Cromer", xlabel="Time (s)", ylabel="Total Linear Momentum (kg*km/s)")


    # Plots all the data on separate lines, ensuring labels and colours are correct.
    ax[1,0].plot(times,Lin_EC_2500_list,label = "2500(s) dT ",color = "black")
    ax[1,0].plot(times,Lin_EC_5000_list,label = "5000(s) dT",color = "blue")
    ax[1,0].plot(times,Lin_EC_10000_list,label = "10000(s) dT",color = "red")
    ...
    for itemRow in DataIn2500:
        En_EC_2500_list.append(itemRow[1])

    for itemRow in DataIn5000:
        En_EC_5000_list.append(itemRow[1])

    for itemRow in DataIn10000:
        En_EC_10000_list.append(itemRow[1])

    # Ensuring the arrays are numpy arrays for ease of handling.
    En_EC_100_list = np.array(En_EC_2500_list)
    En_EC_1000_list = np.array(En_EC_5000_list)
    En_EC_10000_list = np.array(En_EC_10000_list)

    # Formatting the title and the graphs axis, the [1,1] indicates where on the 2x2 this plot will sit.
    ax[1,1].set(title="Total Energy Over 1 Year Euler Cromer", xlabel="Time (s)", ylabel="Total Energy (10^6 J)")

    # Plots all the data on separate lines, ensuring labels and colours are correct.
    ax[1,1].plot(times,En_EC_100_list,label = "2500(s) dT ",color = "black")
    ax[1,1].plot(times,En_EC_1000_list,label = "5000(s) dT",color = "blue")
    ax[1,1].plot(times,En_EC_10000_list,label = "10000(s) dT",color = "red")
    ...
    # Ensuring the arrays are numpy arrays for ease of handling.
    max_yEn10 = np.amax(En_EC_10000_list)
    min_yEn10 = np.amin(En_EC_10000_list)
    max_yEn5 = np.amax(En_EC_5000_list)
    min_yEn5 = np.amin(En_EC_5000_list)
    max_yEn25 = np.amax(En_EC_2500_list)
    min_yEn25 = np.amin(En_EC_2500_list)
    ...
    max_yLM10 = np.amax(Lin_EC_10000_list)
    min_yLM10 = np.amin(Lin_EC_10000_list)
    max_yLM5 = np.amax(Lin_EC_5000_list)
    min_yLM5 = np.amin(Lin_EC_5000_list)
    max_yLM25 = np.amax(Lin_EC_2500_list)
    min_yLM25 = np.amin(Lin_EC_2500_list)
    ...
    print("For steps of 2,500 (s)")
    print(f"The minimum Linear Momentums were as such, Max : {max_yLM25}  Min : {min_yLM25}")
    print(f"With a difference of {round((((max_yLM25-min_yLM25)/max_yLM25)*100),12)}%")

    # For Angular Momentum

    max_yAM10 = np.amax(Ang_EC_10000_list)
    min_yAM10 = np.amin(Ang_EC_10000_list)
    max_yAM5 = np.amax(Ang_EC_5000_list)
    min_yAM5 = np.amin(Ang_EC_5000_list)
    max_yAM25 = np.amax(Ang_EC_2500_list)
    min_yAM25 = np.amin(Ang_EC_2500_list)
    ...
    print("For steps of 2,500 (s)")
    print(f"The minimum Angular Momentums were as such, Max : {max_yAM25}  Min : {min_yAM25}")
